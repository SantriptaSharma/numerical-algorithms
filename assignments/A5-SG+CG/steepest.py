...
def steepest_descent(A: np.ndarray, b: np.ndarray, x0: np.ndarray | None = None, max_iterations = 500) -> tuple[np.ndarray, float, int, list[np.ndarray], list[np.ndarray]]:
	"""
	uses the method of steepest descent to minimise the homogeneous (c = 0) quadratic form given by (A, b)
	...
	for k in range(max_iterations):
		alpha = np.dot(residual, residual) / np.dot(residual, A @ residual)
		x = x + alpha * residual

		xs.append(x)

		# technically can be sped up using the residual update rule, but we care about stability more rn
		residual = -calc_grad(x)
...
if __name__ == "__main__":
	...
	x, val, its, xs, residuals = steepest_descent(A, b, x0)
